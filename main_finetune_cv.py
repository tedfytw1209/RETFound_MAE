...
def main(args, criterion):
    ...
    for fold_idx, (tr_idx, va_idx) in enumerate(skf.split(np.arange(full_pat_len))):
        ...
        if args.eval:
            ...
            wandb_dict={f'CV{fold_idx}/test_{k}': v for k, v in test_stats.items()}
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            if log_writer is not None and misc.is_main_process():
                wandb_dict = {f"CV{fold_idx}/epoch": epoch}
                wandb_dict.update({f'CV{fold_idx}/train_{k}': v for k, v in train_stats.items()})
                wandb_dict.update({f'CV{fold_idx}/val_{k}': v for k, v in val_stats.items()})
            ...
            if epoch == (args.epochs - 1):
                ...
                val_stats, val_score = evaluate(data_loader_val, model, device, args, -1, mode='val',
                                            num_class=args.nb_classes, k=args.num_k, log_writer=log_writer, eval_score=args.eval_score)
                wandb_dict = {}
                wandb_dict.update({f'CV{fold_idx}/best_val_{k}': v for k, v in val_stats.items()})
                wandb.log(wandb_dict)
