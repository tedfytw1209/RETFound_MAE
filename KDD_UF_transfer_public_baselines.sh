...
EPOCHS=50
NUM_CLASS=2
XAI_METHOD=${1:-"hirescam"}  # not used in this script
ADD_WORD1=""
ADD_WORD2=""
...
for i in "${!MODELS[@]}"; do
    ...
    RESUME="${RESULTS_DIR}/${DATASET}-${DATA_TYPE}-all-${FINETUNED_MODEL}-${MODALITY}-bs${BATCH_SIZE}ep${EPOCHS}lr${LR}optadamw-defaulteval-trsub0-${ADD_WORD1}-${ADD_WORD2}/checkpoint-best.pth"
    echo "${RESUME}"
